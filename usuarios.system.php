...
{ 
  ...
  if($usuario<>"" and  $_POST["clave"])
  {
     if($_REQUEST["op"]=="nuevo")
	 {
	   ...
	   {
	     $cadena = "insert into usersession (id,usuario,clave,dni,nivel,nomblargo,'idregion') values (NULL,'$usuario','$clave','$dni','$nivel','$nombre','$iddepartamento')";
		 ...
		 if($clave<>"")
	      {
			...
			$cadena = "update usersession set clave='$clave',dni='$dni',nivel='$nivel',nomblargo='$nombre', idregion = '$iddepartamento' where id=$iduser";
			$conexion = conectar_base();
			//echo $cadena;
			mysql_select_db ("sistemaatencion",$conexion);
			mysql_query($cadena,$conexion);
  ...
  if($_REQUEST["op"]=="editar")
  {
      ...
      $nombre = retorna_cadena($_REQUEST["iduser"],"id",5,"usersession");
      $iddepartamento = retorna_cadena($_REQUEST["iduser"],"id",6,"usersession");
      $user=1;
  }
...
<tr ><td >
<b>Nombre Largo
</td><td>:</td><td><input name='nombre' value="<? echo $nombre; ?>"></td></tr>
<tr ><td >
<b>Nivel
</td><td>:</td><td>
<select name='nivel'>
  <option value="1" <? if($nivel==1){ echo "selected"; } ?> >Administrador</option>
  <option value="2" <? if($nivel==2){ echo "selected"; } ?>>Operador</option>
  <option value="3" <? if($nivel==3){ echo "selected"; } ?>>Visualizador</option>
</select></td></tr>

<tr ><td >
<b>Region
</td><td>:</td><td>
<select name="iddepartamento" id="iddepartamento" value="<? echo $iddepartamento; ?>">
